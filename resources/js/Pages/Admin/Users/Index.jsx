...
export default function UsersIndex({ users, roles, filters }) {
    ...
    const handleFilter = (newStatus, newRole = role) => {
        setStatus(newStatus);
        setRole(newRole);
        ...
        <AdminLayout>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">Manage all user accounts and their roles</p>
                </div>
            </div>
                ...
                {users?.data?.length > 0 ? (
                                        ...
                                        <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 bg-gradient-to-br from-brand to-brand-dark rounded-full flex items-center justify-center text-white font-medium">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex flex-wrap gap-1">
                                                    {user.roles?.map((roleName, idx) => (
                                                        <span
                                                            ...
                                                            className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                                                                roleColors[roleName.toLowerCase()] || 'bg-gray-100 text-gray-700'
                                                            }`}
                                                        >
                                                            {roleName}
                                                        </span>
                                                    ...
                                                    {user.is_seller && (
                                                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${roleColors.seller}`}>
                                                            Seller
                                                        </span>
                                                    )}
                                                    {user.roles?.length === 0 && !user.is_seller && (
                    ...
                    <div className="p-12 text-center">
                        ...
                        <p className="text-gray-500">
                            {search || status || role ? 'No users match your filters.' : 'No users have registered yet.'}
                        </p>
                    </div>
                )}
            </div>
