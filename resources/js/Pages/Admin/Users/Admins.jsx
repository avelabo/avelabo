...
export default function AdminsIndex({ users, roles, filters }) {
    const [status, setStatus] = useState(filters?.status || '');
    const [search, setSearch] = useState(filters?.search || '');

    const handleFilter = (newStatus) => {
        setStatus(newStatus);
        router.get(route('admin.users.admins'), {
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Administrators</h2>
                    <p className="text-gray-500 text-sm">Manage admin user accounts with full system access</p>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                {users?.data?.length > 0 ? (
                            ...
                            <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700">
                                <div className="flex items-center justify-between">
                                    <p className="text-sm text-gray-500">
                                        Showing {users.from} to {users.to} of {users.total} admins
                                    </p>
                    ...
                    <div className="p-12 text-center">
                        <span className="material-icons text-5xl text-gray-300 mb-4">admin_panel_settings</span>
                        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No administrators found</h3>
                        <p className="text-gray-500">
                            {search || status ? 'No administrators match your filters.' : 'No administrators have been added yet.'}
                        </p>
                    </div>
                )}
            </div>
